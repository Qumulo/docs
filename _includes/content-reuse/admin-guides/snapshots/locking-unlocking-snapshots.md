...
## Prerequisites
You can lock and unlock snapshots in a number of ways:

* **With a Public Key:** In Qumulo Core 6.1.0.3 (and higher), you can [use a public key from the Qumulo file system key store] (../encryption-data-security/managing-security-keys.html).

* **With a Snapshot Policy:** In Qumulo Core 6.1.0.3 (and higher), you can configure a snapshot policy to [lock all new snapshots by using the same private key](managing-snapshots.html#create-snapshot-with-policy).

* **With a Lock Key:** In Qumulo Core 6.1.1 (and higher), you can configure a replication target relationship to [lock all new policy snapshots by using the same lock key](#replication-target-locking).

{{site.data.alerts.note}}
<ul>
  <li>Unlocking a snapshot requires a cryptographic signature generated by using a private key.</li> 
  <li>While it isn't possible to delete or shorten the expiration time of a locked snapshot it is possible to extend the expiration time of a locked snapshot.</li>
