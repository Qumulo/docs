...
* Unlocking a snapshot requires proving that you can sign a challenge by using the same key that locked the snapshot.
  ...
  To unlock a snapshot, run the {% include qq.html command="snapshot_unlock_snapshot" %} command and specify the snapshot ID and the path to the private key file. For example:
